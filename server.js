  ...
  try {
    ...
    if (!user) {
      ...
      const insert = await db.query(`
        ...
        RETURNING *
      `, [username, googleUser.name, googleUser.email, randomPass]);
...
app.get('/api/auth/validate', authenticate, async (req, res) => {
  try {
    res.json({ user: { id: req.user.id, name: req.user.name, username: req.user.username, role: req.user.role, email: req.user.email } });
  } catch(e) {
